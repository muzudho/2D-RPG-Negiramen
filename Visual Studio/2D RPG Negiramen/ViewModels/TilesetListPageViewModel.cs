...
class TilesetListPageViewModel : ObservableObject, ITilesetListPageViewModel
{
    ...
    public void SaveSelectedTileset(TilesetLocalConfigurationDifference difference)
    {
        ...
        var tilesetUuid = UUID.FromString(this.SelectedTilesetFileStemAsStr);
    ...
    {
        foreach (var tilesetRecordVM in this.TilesetRecordVMList)
        {
            var tilesetUuid = UUID.FromString(tilesetRecordVM.UuidAsStr);

            //FileStem fileStem = FileStem.FromString(tilesetRecordVM.UuidAsStr);
            //var tilesetLocalTomlLocation = App.GetOrLoadProjectConfiguration().UnityAssetsFolderLocation.YourCircleFolder.YourWorkFolder.AutoGeneratedFolder.ImagesFolder.TilesetsFolder.LocalesFolder.CreateSelectedLocaleFolder().CreateTilesetLocalToml(tilesetUuid);
                ...
                out bool isNew))
            {
                ...
                {
                    tilesetRecordVM.TitleAsStr = newConfiguration.Title.AsStr;
                }
            }
        }
