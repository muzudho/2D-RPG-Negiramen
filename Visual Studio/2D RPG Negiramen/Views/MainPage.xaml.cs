    ...
    async void TileCropButton_Clicked(object sender, EventArgs e)
    {
        ...
        await CodeBehindHelper.ReadyGoToNext(
            onOk: async () =>
            {
                ...
                // タイルセット画像ファイル・パス
                var tilesetImageFile = App.GetOrLoadConfiguration().UnityAssetsFolder.YourCircleFolderNameFolder.YourWorkFolderNameFolder.AutoGeneratedFolder.ImagesFolder.TilesetsFolder.CreateTilesetPng(fileStem);

                // タイルセットCSVファイル・パス
                var tilesetSettingsFile = App.GetOrLoadConfiguration().UnityAssetsFolder.YourCircleFolderNameFolder.YourWorkFolderNameFolder.AutoGeneratedFolder.DataFolder.CsvFolder.TilesetsFolder.CreateTilesetCsv(fileStem);
