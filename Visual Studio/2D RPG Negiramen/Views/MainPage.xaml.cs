        ...
        await ReadyGoToNext(
            onOk: async () =>
            {
                ...
                var tilesetImageFile = new Models.FileEntries.Locations.TilesetImageFile(
                    pathSource: FileEntryPathSource.FromString(App.GetOrLoadConfiguration().UnityAssetsFolder.YourCircleNameFolder.YourWorkNameFolder.AutoGeneratedFolder.ImagesFolder.TilesetFolder.CreateTilesetPng(fileStem).Path.AsStr),
                    convert: (pathSource) => FileEntryPath.From(pathSource,
                                                                replaceSeparators: true));

                // タイルセットCSVファイル・パス
                var tilesetSettingsFile = new Models.FileEntries.Locations.TilesetSettingsFile(
                    pathSource: FileEntryPathSource.FromString(App.GetOrLoadConfiguration().UnityAssetsFolder.YourCircleNameFolder.YourWorkNameFolder.AutoGeneratedFolder.DataFolder.CsvFolder.TilesetFolder.CreateTilesetCsv(fileStem).Path.AsStr),
                    convert: (pathSource)=>FileEntryPath.From(pathSource,
                                                              replaceSeparators: true));
