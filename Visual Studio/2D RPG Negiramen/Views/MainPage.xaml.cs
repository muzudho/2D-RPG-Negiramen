    ...
    async void TileCropButton_Clicked(object sender, EventArgs e)
    {
        ...
        await ReadyGoToNext(
            onOk: async () =>
            {
                ...
                // タイルセット画像ファイル・パス
                var tilesetImageFile = App.GetOrLoadConfiguration().UnityAssetsFolder.YourCircleNameFolder.YourWorkNameFolder.AutoGeneratedFolder.ImagesFolder.TilesetFolder.CreateTilesetPng(fileStem);

                // タイルセットCSVファイル・パス
                var tilesetSettingsFile = App.GetOrLoadConfiguration().UnityAssetsFolder.YourCircleNameFolder.YourWorkNameFolder.AutoGeneratedFolder.DataFolder.CsvFolder.TilesetFolder.CreateTilesetCsv(fileStem);
